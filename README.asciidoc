...
[source,shell]
----
$ curl --location https://github.com/metio/ilo/releases/download/2.3.0/ilo-2.3.0-native-unix.zip --output ilo.zip
$ unzip ilo.zip
$ ln --symbolic --relative ilo-2.3.0/ilo ~/bin/ilo
----

Make sure that `~/bin` is in your `PATH`, or adapt the location to your own needs.
...
[source,shell]
----
$ curl --location https://github.com/metio/ilo/releases/download/2.3.0/ilo-2.3.0-java11.zip --output ilo.zip
$ unzip ilo.zip
$ ln --symbolic --relative ilo-2.3.0/bin/ilo ~/bin/ilo
----

Windows users have to use `ilo.bat` instead of `ilo` and put the `.bat` file in their `PATH`.
