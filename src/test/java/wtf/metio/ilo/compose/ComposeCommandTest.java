...
class ComposeCommandTest extends TestMethodSources {
  ...
  void dockerLikeWithDefaults(final String runtime) {
    ...
    options.service = "dev";
    // -T (disable pseudo-TTY) is suppressed only when attached to a real terminal, which the test
    // JVM is not, so an interactive run still gets -T here.
    assertCommandLine(
        List.of(),
        List.of(),
        call(tool, "--file", DOCKER_COMPOSE_YML, "run", "-T", options.service),
        call(tool, "--file", DOCKER_COMPOSE_YML, "down"));
  }
